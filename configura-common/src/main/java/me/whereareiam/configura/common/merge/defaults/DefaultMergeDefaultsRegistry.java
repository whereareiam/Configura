...
public final class DefaultMergeDefaultsRegistry {
	private final Map<Class<?>, MergeDefaultsProvider<?>> providers = new ConcurrentHashMap<>();

	public DefaultMergeDefaultsRegistry copy() {
		DefaultMergeDefaultsRegistry copy = new DefaultMergeDefaultsRegistry();
		copy.providers.putAll(this.providers);
		return copy;
	}

	public <T, P extends MergeDefaultsProvider<T>> void registerDefaults(Class<P> providerClass) {
	...
	public <T> MergeDefaultsProvider<T> getDefaultsProvider(Class<T> modelType) {
		return (MergeDefaultsProvider<T>) providers.get(modelType);
	}

	public Map<Class<?>, MergeDefaultsProvider<?>> asMap() {
